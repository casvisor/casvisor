...
func (p *LocalFileSystemStorageProvider) ListObjects(dir string) ([]*Object, error) {
	objects := []*Object{}
	fullPath := p.path
...
func (p *LocalFileSystemStorageProvider) Mkdir(key string) error {
	return os.MkdirAll(filepath.Join(p.path, key), os.ModePerm)
}
