...
class PatientListPage extends BaseListPage {
  ...
  addPatient() {
    ...
    PatientBackend.addPatient(newPatient)
      .then((res) => {
        if (res.status === "ok") {
          this.props.history.push({pathname: `/patients/${newPatient.owner}/${newPatient.name}`, mode: "add"});
          Setting.showMessage("success", "Patient added successfully");
        } else {
          Setting.showMessage("error", `Failed to add Patient: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Patient failed to add: ${error}`);
      });
  }
  ...
  deletePatient(i) {
    PatientBackend.deletePatient(this.state.data[i])
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", "Patient deleted successfully");
          this.setState({
          ...
          Setting.showMessage("error", `Failed to delete Patient: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Patient failed to delete: ${error}`);
      });
  }
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <Link to={`/patients/${record.owner}/${record.name}`}>{text}</Link>
          );
        },
      },
      {
          ...
          return (
            <div>
              <Button
                ...
                onClick={() => this.props.history.push(`/patients/${patient.owner}/${patient.name}`)}
