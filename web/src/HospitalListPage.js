...
class HospitalListPage extends BaseListPage {
  ...
  addHospital() {
    ...
    HospitalBackend.addHospital(newHospital)
      .then((res) => {
        if (res.status === "ok") {
          this.props.history.push({pathname: `/hospitals/${newHospital.owner}/${newHospital.name}`, mode: "add"});
          Setting.showMessage("success", "Hospital added successfully");
        } else {
          Setting.showMessage("error", `Failed to add Hospital: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Hospital failed to add: ${error}`);
      });
  }
  ...
  deleteHospital(i) {
    HospitalBackend.deleteHospital(this.state.data[i])
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", "Hospital deleted successfully");
          this.setState({
          ...
          Setting.showMessage("error", `Failed to delete Hospital: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Hospital failed to delete: ${error}`);
      });
  }
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <Link to={`/hospitals/${record.owner}/${record.name}`}>{text}</Link>
          );
        },
      },
      {
          ...
          return (
            <div>
              <Button
                ...
                onClick={() => this.props.history.push(`/hospitals/${hospital.owner}/${hospital.name}`)}
