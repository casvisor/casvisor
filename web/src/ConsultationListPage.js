...
class ConsultationListPage extends BaseListPage {
  ...
  addConsultation() {
    ...
    ConsultationBackend.addConsultation(newConsultation)
      .then((res) => {
        if (res.status === "ok") {
          this.props.history.push({pathname: `/consultations/${newConsultation.owner}/${newConsultation.name}`, mode: "add"});
          Setting.showMessage("success", "Consultation added successfully");
        } else {
          Setting.showMessage("error", `Failed to add Consultation: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Consultation failed to add: ${error}`);
      });
  }
  ...
  deleteConsultation(i) {
    ConsultationBackend.deleteConsultation(this.state.data[i])
      .then((res) => {
        if (res.status === "ok") {
          Setting.showMessage("success", "Consultation deleted successfully");
          this.setState({
          ...
          Setting.showMessage("error", `Failed to delete Consultation: ${res.msg}`);
        }
      })
      .catch(error => {
        Setting.showMessage("error", `Consultation failed to delete: ${error}`);
      });
  }
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <Link to={`/consultations/${record.owner}/${record.name}`}>{text}</Link>
          );
        },
      },
      {
          ...
          return (
            <div>
              <Button
                ...
                onClick={() => this.props.history.push(`/consultations/${consultation.owner}/${consultation.name}`)}
